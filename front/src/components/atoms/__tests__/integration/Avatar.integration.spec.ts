...
describe('Avatar Integration', () => {
  ...
  describe('responsive behavior', () => {
    it('should handle different sizes appropriately', () => {
      ...
      expect(window.getComputedStyle(smallElement).width)
        .toBe(getComputedStyle(document.documentElement).getPropertyValue('--spacing-xl'))
      
      expect(window.getComputedStyle(mediumElement).width)
        .toBe(getComputedStyle(document.documentElement).getPropertyValue('--spacing-2xl'))
        
      expect(window.getComputedStyle(largeElement).width)
        .toBe(getComputedStyle(document.documentElement).getPropertyValue('--spacing-3xl'))
    })
  })
}) 