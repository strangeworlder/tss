...
describe('BlogPostTitle', () => {
  ...
  it('renders correctly with a long title', () => {
    ...
    expect(wrapper.text()).toContain(
      'This is a very long blog post title that should be properly displayed',
    )
  })
  ...
  it('applies the correct CSS classes based on the variant', () => {
    const fullWrapper = mountBlogPostTitle(
      createDefaultBlogPostTitleProps({ variant: BlogPostTitleVariantEnum.FULL }),
    )
    expect(fullWrapper.classes()).toContain('blog-post-title--full')

    const compactWrapper = mountBlogPostTitle(
      createDefaultBlogPostTitleProps({ variant: BlogPostTitleVariantEnum.COMPACT }),
    )
