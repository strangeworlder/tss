...
describe('NotificationList', () => {
  ...
  it('renders properly with notifications', () => {
    // Mock the store with default notifications
  ...
  it('handles empty state correctly', () => {
    // Mock the store with empty notifications
    vi.mocked(useNotificationStore).mockReturnValue({
      notifications: [],
      removeNotification: vi.fn(),
      addNotification: vi.fn(),
  ...
  it('renders different notification types correctly', () => {
    // Mock the store with all notification types
    vi.mocked(useNotificationStore).mockReturnValue({
      notifications: mockNotificationLists.allTypes.notifications,
      removeNotification: vi.fn(),
      addNotification: vi.fn(),
      success: vi.fn(),
      error: vi.fn(),
  ...
  it('handles long messages correctly', () => {
    // Mock the store with long messages
    vi.mocked(useNotificationStore).mockReturnValue({
      notifications: mockNotificationLists.longMessages.notifications,
      removeNotification: vi.fn(),
      addNotification: vi.fn(),
      success: vi.fn(),
      error: vi.fn(),
    ...
    expect(items[0].text()).toContain(
      'This is a very long success message that should be truncated'
    );
    expect(items[1].text()).toContain('This is a very long error message that should be truncated');
  });
  ...
  it('verifies responsive behavior', async () => {
    ...
    expect(getComputedStyle(list.element).maxWidth).toBe('var(--notification-max-width)');
  });
});
