...
const createDummyBlogPosts = async (): Promise<void> => {
  const techTags = ['javascript', 'typescript', 'vue', 'react', 'node', 'express', 'mongodb', 'postgresql', 'graphql', 'docker', 'aws', 'frontend', 'backend', 'fullstack', 'api'];
  ...
  for (let i = 0; i < 10; i++) {
    // Create a title
    const title = faker.lorem.words(faker.datatype.number({ min: 3, max: 6 })).split(' ')
